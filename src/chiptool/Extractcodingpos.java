 ...
 * @author jnu
 */
public class Extractcodingpos extends Thread {
    //arrays for data
    char[] genome;
    int[] chip;
    int[] ann;
    ...
    public Extractcodingpos(char[] g, int[] a, int[] c, boolean nc) {
        genome = g;
        chip = c;
        ann = a;
        ncflag = nc;
...
public void run() {
        ...
        for (int i = 0; i < codpos.length - 1; i = i + 2) {
            temp = String.valueOf(">"+codpos[i]) + "\t" + String.valueOf(codpos[i + 1]);
            
            for (int j = codpos[i]; j <= codpos[i + 1]; j++) {
